...
def test_portfolio_project_is_grounded():
    result = build_portfolio().handle("Cuál es el avance de PRJ-001", {})
    assert result.status == "grounded"
    assert result.evidence[0].source_id == "PRJ-001"
    assert "68%" in result.summary


def test_portfolio_context_can_select_project():
    result = build_portfolio().handle("Cuál es su próximo hito", {"project_id": "PRJ-002"})
    assert result.status == "grounded"
    assert result.evidence[0].source_id == "PRJ-002"
